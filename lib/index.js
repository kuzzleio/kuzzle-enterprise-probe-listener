...
No measures will be sent to probes.`);
  ...
  _configureProbes(probes) {
    ...
    for (const name of Object.keys(probes)) {
      ...
      if (!probe.type) {
        console.error(`plugin-probe: [probe: ${name}] "type" parameter missing"`);
        continue;
      }

      if (['counter', 'monitor', 'watcher', 'sampler'].indexOf(probe.type) === -1) {
        console.error(`plugin-probe: [probe: ${name}] type "${probe.type}" is not supported"`);
        continue;
      }

      if (probe.type === 'monitor' && (!probe.hooks || !Array.isArray(probe.hooks))) {
        console.error(`plugin-probe: [probe: ${name}] Configuration error: missing "hooks"`);
        continue;
      }
      ...
      if (probe.type === 'counter') {
        if (!probe.increasers && !probe.decreasers) {
          console.error(`plugin-probe: [probe: ${name}] Configuration error: missing "increasers" or "decreasers"`);
          continue;
        }

        if (probe.increasers && !Array.isArray(probe.increasers)) {
          console.error(`plugin-probe: [probe: ${name}] Configuration error: "increasers" must be an array`);
          continue;
        }

        if (probe.decreasers && !Array.isArray(probe.decreasers)) {
          console.error(`plugin-probe: [probe: ${name}] Configuration error: "decreasers" must be an array`);
          continue;
        }

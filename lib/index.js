...
class ProbeListenerPlugin {
  ...
  init(customConfig, context) {
    ...
    return this.waitForKdc(config.kdcHost, config.kdcPort, config.maxTriesToConnect)
      .then(() => {
        return new Bluebird((resolve, reject) => {
          this.client = new Kuzzle(config.kdcHost, {
            autoQueue: true,
            autoReplay: true,
            port: config.kdcPort
          }, (err) => {
            ...
            debug('===> Probes listener Initialized!');
            resolve();
          });
        });
      });
  ...
  /**
   * Waits for KDC to be up by polling every second.
   *
   * @param  {String} kdcHost
   * @param  {Number} kdcPort
   * @param  {Number} maxTries The maximum allowed number of tries before KDC
   *                           is considered down.
   * @param  {Number} tryCount The current try count (recursion only)
   * @return {Promise}
   */
  waitForKdc(kdcHost, kdcPort, maxTries = 10, tryCount = 0) {
    const kdcUrl = `http://${kdcHost}:${kdcPort}`;
    return axios
      .get(kdcUrl)
      .then(() => debug(`KDC is responding at ${kdcUrl}`))
      .catch(() => {
        if (tryCount > maxTries) {
          return Bluebird.reject(`Unable to connect to KDC at ${kdcUrl}. Timeout expired.`);
        }
        debug(`KDC not responding at ${kdcUrl}, retrying...`);
        return Bluebird.delay(1000)
          .then(() => this.waitForKdc(kdcHost, kdcPort, maxTries, tryCount + 1));
      });
  }
